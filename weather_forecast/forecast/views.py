    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                weather_data = WeatherData.objects.filter(latitude=latitude, longitude=longitude, detailing_type=detailing_type).latest('last_updated')
                
                # Check if the retrieved data is still valid based on expiry settings
                if (timezone.now() - weather_data.last_updated).total_seconds() / 60 <= settings.DATA_EXPIRY_MINUTES:
            ...
            if fetched_data:
                ...
                WeatherData.objects.create(latitude=latitude, longitude=longitude, detailing_type=detailing_type, data=fetched_data)
                return Response(fetched_data)
            else:
                # If fetching data from the API fails, return an error response
                return Response("Failed to fetch weather data from OpenWeatherMap API", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        else:
