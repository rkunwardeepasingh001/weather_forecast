...
class WeatherDataSerializer(serializers.Serializer):
    ...
    def to_internal_value(self, data):    
        try:
            ...
            raise serializers.ValidationError({"error": "Invalid latitude or longitude"})

    # Validate latitude to ensure it's within the range [-90, 90]
    def validate_latitude(self, value):
        if value < -90 or value > 90:
            raise serializers.ValidationError("Latitude must be within the range -90 to 90")
        return value

    # Validate longitude to ensure it's within the range [-180, 180]
    def validate_longitude(self, value):
        if value < -180 or value > 180:
            raise serializers.ValidationError("Longitude must be within the range -180 to 180")
        return value
